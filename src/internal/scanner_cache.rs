...
//!
//! The scanner cache is a global cache that stores `ScannerNfaImpl`` instances. It is used to
...
/// The cache is a `FxHashMap` that maps a vectors of `ScannerMode` to `Arc<ScannerNfaImpl>`.
pub(crate) struct ScannerCache {
    cache: FxHashMap<Vec<ScannerMode>, Arc<ScannerNfaImpl>>,
}
...
impl ScannerCache {
    ...
    pub(crate) fn get(&mut self, modes: &[ScannerMode]) -> Result<ScannerNfaImpl> {
        if let Some(scanner) = self.cache.get(modes) {
            ...
            // scanner.
            // The clone is much cheaper than creating a new scanner.
            let cloned_scanner =
                unsafe { (*std::sync::Arc::<ScannerNfaImpl>::as_ptr(scanner)).clone() };
            Ok(cloned_scanner)
        } else {
            let scanner: Arc<ScannerNfaImpl> = Arc::new(modes.try_into()?);
            self.cache.insert(modes.to_vec(), scanner.clone());
